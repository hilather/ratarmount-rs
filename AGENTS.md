...
| Local cached sidecar reused after remote archive replace (URL-label tarstats no-op) | `cargo test -p ratarmount --bin ratarmount apply_remote_index_local_copy_rejects` · `cargo test -p ratarmount --bin ratarmount cached_remote_index_is_stale_size` |
