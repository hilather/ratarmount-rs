...
| Nested UDF no-tmp / NSR02/NSR03 @ 32 KiB (F-8 crate) | `cargo test -p ratarmount-formats-udf --lib` (always-on synthetic VRS + volume; ISO `CD001`-only negative; mixed-disc NSR still true; `mkudffs`/`mkisofs` skip if missing) |
