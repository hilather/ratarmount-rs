...
| restic snapshot browser (two snapshots, 1 KiB pack read, password Debug) | `cargo test -p ratarmount-formats-restic --lib restic` · `cargo test -p ratarmount-session --lib restic_open` |
...
| SMB QUERY_DIRECTORY Depth-1 names + sizes | `cargo test -p ratarmount-remote --lib smb_listing_query_directory` |
| SMB share listing RemoteFolderMountSource | `cargo test -p ratarmount-remote --lib smb_folder_mountsource` |
| SMB file URL dialect miss is Ok(None) (Range/smbclient takeover) | `cargo test -p ratarmount-remote --lib smb_folder_file_url_dialect` |
| Session `smb://` live Range + F-1 folders | `cargo test -p ratarmount-session --lib remote_smb` · `cargo test -p ratarmount-session --lib try_open_remote_folder_url` |
...
| Extract short/truncated member reports success (silent dest truncate) | `cargo test -p ratarmount-session --lib extract_to_short_member` |
