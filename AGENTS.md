...
| NTFS RO list/read / nested no-tmp / offset (F-8) | `cargo test -p ratarmount-formats-ntfs --lib looks_like` · `cargo test -p ratarmount-formats-ntfs --lib open_from_reader` · `cargo test -p ratarmount-formats-ntfs --lib list_dirents` · `cargo test -p ratarmount-formats-ntfs --lib filetime` |
