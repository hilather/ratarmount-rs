...
| UDIF `koly` parse / inner FAT no-tmp (F-8 DMG crate; HFS+ residual) | `cargo test -p ratarmount-formats-dmg --lib` (always-on synthetic koly + raw/ADC/zlib/bzip2 + FAT-in-UDIF; `hdiutil` skip) |
