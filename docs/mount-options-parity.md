...
| `--commit-overlay` | yes | yes (TAR + gzip/bzip2/xz via GNU tar; `.tar.zst` splice including earlier-frame delete and later complete-TAR-frame delete without `-i`; ZIP full rebuild; create-if-missing for uncompressed `.tar` only) | `[x]` / residual encrypted ZIP |
| `--commit-overlay-on-exit` | no | **added** (uncompressed TAR or `.tar.zst` + durable `-w`; same create-if-missing as `-w`; last-frame zstd rewrite — does not recompress the prefix; persist still copies the compressed prefix; on-disk sidecar is patched so remount does not rescan prefix frames; `:memory:` still full-rebuild; 2× compressed disk headroom; never refuse on size; warn when last-frame uncompressed > 64 MiB; gzip rejected; live still **rejects** prefix-frame mutate — offline `--commit-overlay` is the zstd escape hatch; later-frame deletes classify past per-frame TAR EOF even when the remount omitted `-i`; SIGINT/SIGTERM / serve return; **`s3://` TAR/ZST** write-probe + spool + multipart PUT + blob/pointer, reopen live Range; anonymous/missing keys exit 2) | `[x]` Rust-only |
| `--commit-overlay-interval DURATION` | no | **added** (`0` off; `2s`/`15m`/`1h`; uncompressed TAR or `.tar.zst` + durable `-w`; same create-if-missing as `-w`; in-process; commits overlay files whose host mtime is at least DURATION old **and** that have no open write fd, not a dump of every overlay file on the clock; same last-frame cost model; later complete-TAR-frame deletes persist even if the remount omitted `-i`; **`s3://` TAR/ZST** same write-through as on-exit) | `[x]` Rust-only |
| `--repack-seekable IN OUT` | no | **added** (offline producer: framed zstd + official seek table; `num_args = 2` so clap cannot steal a mountpoint; IN OUT must immediately follow the flag; helpers go before it or after the paths. Local files only; exclusive with `--nfs`/exports/`-w`/FUSE mountpoint. Helpers: `--repack-frame-size` default `8M`, `--repack-level` default 3, `--repack-keep-gzip` RGZI sidecar, `--repack-gzidx`, `--repack-force`. [guide](https://github.com/hilather/ratarmount-rs/blob/main/docs/zstd-random-access.md)) | `[x]` Rust-only |
| `-p` / `--prefix` | yes | yes | `[x]` |
...
| `--sftp` | no | **added** (`--features sftp-russh`; packages enable it; else exit 2; password env `RATARMOUNT_SFTP_PASSWORD` / `_USER`) | `[x]` / russh MSRV 1.85 feature note |
| `--sftp-bind [host:]port` | no | **added** (IPv4 only; default `127.0.0.1:20222`; required value; port 22 is this flag) | `[x]` |
...
| `--publish-index` / `--publish-index-to PATH` | no | **added** (boolean + required-value dest; local copy of 0.7.x sidecar; always writes `{archive}.index.ptr`; keep-last-K=2 snapshots when a pointer exists; `s3://` PUTs blob then `{url}.index.ptr`; OCI referrer PUT residual; GCS/Azure PUT residual) | `[x]` Rust-only |
...
- Export flags (`--http` / `--webdav` / `--smb` / `--ninep` / `--sftp` / `--sftp-subsystem`) are booleans (`ArgAction::SetTrue`) with required-value binds (`num_args = 1`). There is no `serve` subcommand and no `--fuse` flag. See [`export.md`](export.md).
