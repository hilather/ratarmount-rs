...
| `--commit-overlay-interval DURATION` | no | **added** (`0` off; `2s`/`15m`/`1h`; uncompressed TAR or `.tar.zst` + durable `-w`; same create-if-missing as `-w`; in-process; commits overlay files whose host mtime is at least DURATION old, not a dump of every overlay file on the clock; same last-frame cost model) | `[x]` Rust-only |
| `-p` / `--prefix` | yes | yes | `[x]` |
