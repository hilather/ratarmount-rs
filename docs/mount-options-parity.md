...
| `--repack-seekable IN OUT` | no | **added** (offline producer: framed zstd + official seek table; `num_args = 2` so clap cannot steal a mountpoint; local files only; exclusive with `--nfs`/exports/`-w`/FUSE mountpoint. Helpers: `--repack-frame-size` default `8M`, `--repack-level` default 3, `--repack-keep-gzip` RGZI sidecar, `--repack-gzidx`, `--repack-force`. [guide](https://github.com/hilather/ratarmount-rs/blob/main/docs/zstd-random-access.md)) | `[x]` Rust-only |
| `-p` / `--prefix` | yes | yes | `[x]` |
