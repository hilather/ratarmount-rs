...
| `--smb` | no | **added** (userspace SMB 2.0.2; share `--smb-share` default `ratarmount`; NTLMv2 + signing when password set) | `~` encrypt / 3.1.1 / Finder |
...
| SMB 2.0.2 export (`--smb`) | no | userspace subset; guest `smbclient -N` unsigned; password env signs | `~` encrypt / 3.1.1 / Finder |
