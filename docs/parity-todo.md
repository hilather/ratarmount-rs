...
| AutoMount recursive (`-r`) | yes | nested no-tmp for TAR/ZIP/7z/`.tar.gz`/CPIO/AR/ISO/WARC/ASAR/XAR/CAB·MSZIP/SQLAR/FAT + TAR flatten; eager same-dir parallel nested opens (FR-6 / #80, `parallel_nested_threads`); see [`embedded-nested-archives.md`](embedded-nested-archives.md) | `[x]` / `~` CAB LZX, SquashFS, RAR nested still spool; CLI wire for nested thread cap residual |
