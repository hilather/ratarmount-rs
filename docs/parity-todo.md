...
| NFSv4.1 userspace export | no | yes (`embednfs` 0.4.1, `--nfs --nfs-vers 4`, `--features nfsv4`, rustc ≥ 1.88; lookup/read/readdir + `-w` overlay writes) | `~` / Linux kernel client unverified; no Kerberos/LAN; rename/symlink `ReadOnly` |
