//! Per-fileid reader LRU + fill-loop / readahead (copied from NFS / FUSE).
//!
//! Stateful protocols (SMB/9P/SFTP, NFS) keep a live `ArchiveRead` per fileid
//! so gzip / solid 7z `cat` does not reopen from 0 on every READ. HTTP may
//! omit the LRU if each GET opens, fill-loops, and drops.
...
fn readahead_fill(
    ...
    if readahead_bytes == 0 {
        ...
        let n = fill_read(reader, &mut buf)?;
        buf.truncate(n);
        return Ok(buf);
    }
    if size == 0 {
    ...
    let n = fill_read(reader, &mut data)?;
    data.truncate(n);
...
/// Open member reader plus sequential readahead state.
pub struct SourceReadState {
    pub reader: Box<dyn ratarmount_core::ArchiveRead>,
...
impl ReaderLru {
    ...
    /// Same as [`Self::new`], plus idle-TTL eviction on insert.
    ...
    pub fn sweep_if_generation_advanced(&self, source: &dyn MountSource, inodes: &InodeTable) {
    ...
    /// exempt: idle sweep drops them too. Cap-pressure eviction still prefers
    /// unpinned slots first (`evict_one`). Next READ re-opens.
    ///
...
fn evict_idle_locked(map: &mut HashMap<u64, ReaderSlot>, now: Instant, ttl: Duration) -> usize {
    ...
    map.retain(|&id, slot| {
        ...
        if slot.pin {
            log::info!("export idle-TTL dropped pinned reader fileid={id}");
        } else {
            log::debug!("export idle-TTL dropped reader fileid={id}");
        }
        dropped += 1;
        false
    });
...
/// Fill from a cached reader slot (readahead + [`fill_read`] loop).
...
mod tests {
    ...
    /// Regression: unknown fileid is NotFound so adapters can map STALE.
    #[test]
    ...
    #[test]
    fn fill_from_state_loops_short_reads() {
        let state = Arc::new(Mutex::new(SourceReadState {
            reader: Box::new(ShortRead(Cursor::new(b"hello!".to_vec()))),
            readahead: ReadaheadState::default(),
        }));
        let out = fill_from_state(&state, 0, 0, 6).unwrap();
